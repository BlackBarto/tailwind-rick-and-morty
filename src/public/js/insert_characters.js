...
export const insertCharacters = characters => {
    ...
    characters.forEach( ({name, status, img, origin, episodes, location, species}) => {
        ...
        <h4 class="order-1 res:order-none font-bold text-center leading-loose self-center justify-self-center text-blue-700 dark:text-sky-200 px-5 text-2xl res:text-3xl row-span-1 col-start-1 col-end-2">${name}</h4>
...
export const getCharacters = () => {
    ...
    fetch(nextPage).then(res => res.json()).then( ({info: {next}, results}) => {
        ...
        if(!loadingCharacters.classList.contains("hidden")) loadingCharacters.classList.add("hidden", "res:hidden")
